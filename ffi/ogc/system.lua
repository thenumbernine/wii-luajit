...
enum { SYS_RESTART = 0 };			/*!< Reboot the gamecube, force, if necessary, to boot the IPL menu. Cold reset is issued */
enum { SYS_HOTRESET = 1 };			/*!< Restart the application. Kind of softreset */
enum { SYS_SHUTDOWN = 2 };			/*!< Shutdown the thread system, card management system etc. Leave current thread running and return to caller */

enum { SYS_RETURNTOMENU = 3 };			/*!< Directly load the Wii Channels menu, without actually cold-resetting the system */
enum { SYS_POWEROFF = 4 };			/*!< Powers off the Wii, automatically choosing Standby or Idle mode depending on the user's configuration */
enum { SYS_POWEROFF_STANDBY = 5 };			/*!< Powers off the Wii to standby (red LED, WC24 off) mode. */
enum { SYS_POWEROFF_IDLE = 6 };			/*!< Powers off the Wii to idle (yellow LED, WC24 on) mode. */
...
enum { SYS_PROTECTCHAN0 = 0 };			/*!< OS memory protection channel 0 */
enum { SYS_PROTECTCHAN1 = 1 };			/*!< OS memory protection channel 1 */
enum { SYS_PROTECTCHAN2 = 2 };			/*!< OS memory protection channel 2 */
enum { SYS_PROTECTCHAN3 = 3 };			/*!< OS memory protection channel 2 */
enum { SYS_PROTECTCHANMAX = 4 };			/*!< _Termination */
...
enum { SYS_PROTECTNONE = 0x00000000 };		/*!< Read and write operations on protected region is granted */
enum { SYS_PROTECTREAD = 0x00000001 };		/*!< Read from protected region is permitted */
enum { SYS_PROTECTWRITE = 0x00000002 };		/*!< Write to protected region is permitted */
enum { SYS_PROTECTRDWR = (SYS_PROTECTREAD|SYS_PROTECTWRITE) };	/*!< Read and write operations on protected region is permitted */

/*!
 *@}
 */

enum { SYS_FONTSIZE_ANSI = (288 + 131072) };
enum { SYS_FONTSIZE_SJIS = (3840 + 1179648) };
...
//enum { MEM_VIRTUAL_TO_PHYSICAL(x) = (((u32)(x)) & ~SYS_BASE_UNCACHED) };									/*!< Cast virtual address to physical address, e.g. 0x8xxxxxxx -> 0x0xxxxxxx */
//enum { MEM_PHYSICAL_TO_K0(x) = (void*)((u32)(x) + SYS_BASE_CACHED) };									/*!< Cast physical address to cached virtual address, e.g. 0x0xxxxxxx -> 0x8xxxxxxx */
//enum { MEM_PHYSICAL_TO_K1(x) = (void*)((u32)(x) + SYS_BASE_UNCACHED) };								/*!< Cast physical address to uncached virtual address, e.g. 0x0xxxxxxx -> 0xCxxxxxxx */
//enum { MEM_K0_TO_PHYSICAL(x) = (void*)((u32)(x) - SYS_BASE_CACHED) };									/*!< Cast physical address to cached virtual address, e.g. 0x0xxxxxxx -> 0x8xxxxxxx */
//enum { MEM_K1_TO_PHYSICAL(x) = (void*)((u32)(x) - SYS_BASE_UNCACHED) };								/*!< Cast physical address to uncached virtual address, e.g. 0x0xxxxxxx -> 0xCxxxxxxx */
//enum { MEM_K0_TO_K1(x) = (void*)((u32)(x) + (SYS_BASE_UNCACHED - SYS_BASE_CACHED)) };			/*!< Cast cached virtual address to uncached virtual address, e.g. 0x8xxxxxxx -> 0xCxxxxxxx */
//enum { MEM_K1_TO_K0(x) = (void*)((u32)(x) - (SYS_BASE_UNCACHED - SYS_BASE_CACHED)) };			/*!< Cast uncached virtual address to cached virtual address, e.g. 0xCxxxxxxx -> 0x8xxxxxxx */
...
function system.MEM_VIRTUAL_TO_PHYSICAL(x) return bit.band(ffi.cast('u32', x), bit.not(ffi.C.SYS_BASE_UNCACHED)) end	--								/*!< Cast virtual address to physical address, e.g. 0x8xxxxxxx -> 0x0xxxxxxx */
function system.MEM_PHYSICAL_TO_K0(x) return ffi.cast('void*', ffi.cast('u32', x) + SYS_BASE_CACHED) end	--									/*!< Cast physical address to cached virtual address, e.g. 0x0xxxxxxx -> 0x8xxxxxxx */
function system.MEM_PHYSICAL_TO_K1(x) return ffi.cast('void*', ffi.cast('u32', x) + SYS_BASE_UNCACHED) end	--								/*!< Cast physical address to uncached virtual address, e.g. 0x0xxxxxxx -> 0xCxxxxxxx */
function system.MEM_K0_TO_PHYSICAL(x) return ffi.cast('void*', ffi.cast('u32', x) - SYS_BASE_CACHED) end	--									/*!< Cast physical address to cached virtual address, e.g. 0x0xxxxxxx -> 0x8xxxxxxx */
function system.MEM_K1_TO_PHYSICAL(x) return ffi.cast('void*', ffi.cast('u32', x) - SYS_BASE_UNCACHED) end	--								/*!< Cast physical address to uncached virtual address, e.g. 0x0xxxxxxx -> 0xCxxxxxxx */
function system.MEM_K0_TO_K1(x) return ffi.cast('void*', ffi.cast('u32', x) + (SYS_BASE_UNCACHED - SYS_BASE_CACHED)) end	--			/*!< Cast cached virtual address to uncached virtual address, e.g. 0x8xxxxxxx -> 0xCxxxxxxx */
function system.MEM_K1_TO_K0(x) return ffi.cast('void*', ffi.cast('u32', x) - (SYS_BASE_UNCACHED - SYS_BASE_CACHED)) end	--			/*!< Cast uncached virtual address to cached virtual address, e.g. 0xCxxxxxxx -> 0x8xxxxxxx */

return setmetatable(system, {__index=ffi.C})
